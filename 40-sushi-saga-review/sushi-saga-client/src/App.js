...
class App extends Component {
  constructor(){
    ...
    this.state = {
      ...
      sushiIndex: 0,
      // Keep track of how much money the user has left - they should only be able to eat sushis they can afford
      money: 100
    }
  }
  ...
  eatSushi = (id, price) => {
    // Only add this sushi object's id to the list of eatenSushis if it isn't in there already and if the current state of money is greater than or equal to the price of the sushi object i.e. don't let the state of money drop below 0
    if (!this.state.eatenSushis.includes(id) && this.state.money >= price) {
      this.setState({
        ...
        // Decrease the state of money by the price of the eaten sushi
  ...
  render() {
    // Get the current 4 sushis we need to render so that we can pass them down to SushiContainer as a prop
    const sushisToRender = this.getSushisToRender()
    return (
      <div className="app">
        ...
        <SushiContainer sushis={sushisToRender} increaseSushiIndex={this.increaseSushiIndex} eatSushi={this.eatSushi} eatenSushis={this.state.eatenSushis} />
        {/* Pass down current state of money and the list of eatenSushis in state as props */}
